...
const SimpleTable = () => {
    ...
    useEffect(() => {
        const fetchContacts = async () => {
            try {
                ...
                setContacts(data.contacts); // Assuming API response contains 'contacts' array
            } catch (error) {
    ...
    return (
        <div className={`container mx-auto p-4 ${cairo100.className}`}>
            <Link href={'/Dashbord/addContacts'} className={`text-right block mb-6 text-blue-600`}>
                <span className={`text-2xl ${cairo200.className} bg-sky-700 text-white py-1 px-5 rounded-2xl`}>اضافة حجز</span>
            </Link>
                                    ...
                                    {contacts.map((contact) => (
                                        <tr key={contact._id} className={`border-b border-gray-200 text-white ${cairo100.className}`}>
                                            <td className="mr-3 py-4 flex space-x-4 items-center pr-6">
                                                ...
                                                <Link href={`/Dashbord/EditContacts/${contact._id}`} className='bg-green-600 text-white px-5 py-2 rounded-lg hover:bg-green-700 transition'>
                                            ...
                                            <td className="pl-14 py-4">{contact.message}</td>
